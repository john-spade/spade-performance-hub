...
export default function EvaluationDetails() {
    ...
    const downloadPDF = () => {
        ...
        doc.text(`Action: ${rec.action}`, 140, 69); // Changed 'Status' to 'Action'
        doc.setTextColor(0, 0, 0);
        ...
        <div className="min-h-screen bg-dark-900 flex">
            ...
            <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-dark-800 border-r border-dark-700 transform transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 lg:static`}>
                <div className="flex flex-col h-full">
                    ...
                    {/* Navigation Links - Linking back to Dashboard Tabs */}
                    <nav className="flex-1 p-4 space-y-2">
                        <button
                            onClick={() => navigate('/client')}
                            className="w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors text-gray-400 hover:bg-dark-700"
                        >
                        ...
                        <button
                            onClick={() => navigate('/client')}
                            className="w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors text-gold-500 bg-gold-500/20"
                        >
